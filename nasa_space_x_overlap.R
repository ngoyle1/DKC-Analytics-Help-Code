    ...
    for(i in 1:nrow(education)){
        education_vector[i] <- fbad_reachestimate(targeting_spec = list(
            ...
            flexible_spec = list(
                ...
                list(interests = data.frame(
                    id = c("6003865907678","6003133848100"),
                    name = c("NASA","SpaceX")
                ))
            )
        ))$users
...
fb_ad_top_dmas <- function(id_vector, name_vector){
    suppressMessages(library(readxl))
...
fb_ad_race <- function(id_vector, name_vector){
    ...
    options(scipen = 999)
    ethnic_groups <- read_excel("/Users/francisco06121988/Desktop/code/code/facebook_api_marketing/race_for_fb_ads.xlsx", sheet = 1)
    race_vector <- NULL
