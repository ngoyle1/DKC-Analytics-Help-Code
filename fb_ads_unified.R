...
fb_ad_top_dmas <- function(id_vector, name_vector, platform){
    suppressMessages(library(readxl))
...
fb_ad_industries <- function(id_vector, name_vector, platform){
...
fb_ad_us_audience_deduped <- function(id_vector, name_vector){
  audience <- NULL
  audience <- fbad_reachestimate(targeting_spec = list(
    geo_locations = list(countries = 'US'),
    publisher_platforms = c("instagram","facebook"),
    flexible_spec = list(list(
      interests = data.frame(
        id = as.character(id_vector),
        name = as.character(name_vector)
